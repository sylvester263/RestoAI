...
export default function ScreenshotFrame({ src, alt, className = '' }) {
  return (
    <div className={`flex flex-col overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--surface-2)] shadow-lg ${className}`}>
      ...
      <img src={src} alt={alt} className="min-h-0 w-full flex-1 object-contain" />
    </div>
  );
}

  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[var(--text-primary)]">Tables</h1>
          <p className="text-sm text-[var(--text-secondary)]">Dine-in QR ordering — print a table's QR and stick it on for customers to scan</p>
        </div>
        {branches.length > 1 && (
      ...
      <form onSubmit={handleAddTable} className="card mb-6 flex items-end gap-3">
        <div className="flex-1">
          <label className="mb-1 block text-xs font-medium text-[var(--text-secondary)]">Table number</label>
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
          {tables.map((t) => (
            <div key={t.id} className="card flex flex-col items-center gap-3 text-center">
              <p className="text-lg font-semibold text-[var(--text-primary)]">Table {t.table_number}</p>
              {t.open_session_id ? (
                ...
                <span className="badge bg-gray-100 text-[var(--text-secondary)]">Free</span>
              )}
              <button onClick={() => setQrTable(t)} className="btn-secondary w-full">
                <QrCode className="h-4 w-4" /> Show QR
              </button>
              {t.open_session_id && (
                <button onClick={() => handleCloseSession(t.open_session_id)} className="text-xs text-[var(--text-secondary)] hover:text-red-600">
                  Close session
                </button>
              )}
            </div>
          ...
          {tables.length === 0 && (
            <p className="col-span-full py-12 text-center text-sm text-[var(--text-tertiary)]">No tables yet — add one above</p>
          )}
        </div>
      )}
      ...
      {qrTable && (
        <Modal open={true} onClose={() => setQrTable(null)} title={`Table ${qrTable.table_number}`} size="sm">
          <div className="flex justify-center rounded-lg bg-[var(--surface-2)] p-4">
            <QRCodeSVG value={qrUrl} size={200} />
          </div>
          <p className="mt-3 break-all text-xs text-[var(--text-tertiary)]">{qrUrl}</p>
          <p className="mt-2 text-xs text-[var(--text-secondary)]">{tenant?.name}</p>
        </Modal>
      )}
    </div>
  );
